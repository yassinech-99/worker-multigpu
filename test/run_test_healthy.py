#!/usr/bin/env python3
"""
Runpod Serverless Endpoint Health Check Test Script
This script sends a simple health check request to your deployed Runpod serverless endpoint using the testinput_healthy.json file.
"""
...
class RunpodHealthTester:
    def __init__(self, api_key: str, endpoint_id: str):
        """
        Initialize the Runpod health tester.
    ...
    def send_async_request(self, input_data: Dict[str, Any]) -> str:
        ...
        print(f"🚀 Sending asynchronous health check request to: {url}")
        print(f"📊 Request size: {len(json.dumps(input_data))} characters")
    ...
    def wait_for_completion(self, job_id: str, max_wait_time: int = 300) -> Dict[str, Any]:
        ...
        while time.time() - start_time < max_wait_time:
            ...
            time.sleep(5)  # Wait 5 seconds before checking again (shorter for health check)
        ...
        try:
            ...
            for i, image_data in enumerate(images):
                if isinstance(image_data, dict):
                    ...
                    if not image_type:
                        # If type is not specified, assume base64 if data exists
                        image_type = 'base64' if image_content else ''
...
def main():
    ...
    print("🏥 Runpod Endpoint Health Check Tester")
    print("=" * 60)
    ...
    # Configuration
    test_input_file = Path(__file__).parent / "testinput_healthy.json"
    output_dir = Path(__file__).parent / "results"
    output_dir.mkdir(exist_ok=True)
    
    # Initialize tester
    tester = RunpodHealthTester(api_key, endpoint_id)
    
    # Load test input
    print(f"📂 Loading health check input from: {test_input_file}")
    ...
    if choice == "1":
        # Synchronous request
        print("\n⚡ Running synchronous health check...")
        start_time = time.time()
        result = tester.send_sync_request(input_data)
        elapsed_time = time.time() - start_time
        
        # Save result
        output_file = output_dir / f"health_sync_result_{timestamp}.json"
        ...
        if "error" not in result:
            ...
            if output:
                ...
                for key, value in output.items():
                    if key == 'images':
                        # Skip images list as we already processed them
                        continue
            ...
            print(f"⏱️  Total elapsed time: {elapsed_time:.2f} seconds")
            print(f"⚠️  Endpoint may be unhealthy or experiencing issues")
    
    elif choice == "2":
        # Asynchronous request
        print("\n🔄 Running asynchronous health check...")
        start_time = time.time()
        ...
        if job_id:
            ...
            # Save result
            output_file = output_dir / f"health_async_result_{timestamp}.json"
            tester.save_result(result, str(output_file))
            ...
            elapsed_time = time.time() - start_time
            print(f"\n❌ Failed to submit health check job")
            print(f"⏱️  Total elapsed time: {elapsed_time:.2f} seconds")
            print(f"⚠️  Endpoint may be unreachable")
