#!/usr/bin/env python3
"""
Runpod Serverless Endpoint Test Script for Background Swap - Flux Kontext
This script sends a request to your deployed Runpod serverless endpoint using the testinput_bkgdswap.json file.
"""
...
class RunpodTester:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(f"❌ Error: Unable to read file with any encoding: {e}")
                    sys.exit(1)
        ...
        try:
            ...
            for i, image_data in enumerate(images):
                if isinstance(image_data, dict):
                    # Extract image info
                    filename = image_data.get('filename', f'bkgd_swap_result_{i+1}.png')
    ...
    def process_background_swap_results(self, result: Dict[str, Any], output_dir: Path, timestamp: int) -> None:
        """
        Process and save background swap specific results (latents, masks, etc.).
        ...
        try:
            ...
            # Look for latent conditioning data
            latent_data = None
            if 'latents' in output:
                latent_data = output['latents']
            elif 'latent_conditioning' in output:
                latent_data = output['latent_conditioning']
            elif 'conditioning' in output:
                latent_data = output['conditioning']
            
            if latent_data:
                # Save latent results as JSON
                latent_file = output_dir / f"latent_results_{timestamp}.json"
                with open(latent_file, 'w', encoding='utf-8') as f:
                    json.dump(latent_data, f, indent=2, ensure_ascii=False)
                print(f"🧠 Latent conditioning results saved to: {latent_file}")
            
            # Look for background extraction/manipulation data
            background_data = None
            if 'background' in output:
                background_data = output['background']
            elif 'background_info' in output:
                background_data = output['background_info']
            elif 'background_mask' in output:
                background_data = output['background_mask']
            
            if background_data:
                # Save background processing results as JSON
                background_file = output_dir / f"background_results_{timestamp}.json"
                with open(background_file, 'w', encoding='utf-8') as f:
                    json.dump(background_data, f, indent=2, ensure_ascii=False)
                print(f"🌄 Background processing results saved to: {background_file}")
            ...
            elif 'process_info' in output:
                intermediate_data = output['process_info']
            ...
            print(f"❌ Error processing background swap results: {e}")
...
def main():
    ...
    print("🧪 Runpod Background Swap - Flux Kontext Endpoint Tester")
    print("=" * 65)
    ...
    # Configuration
    test_input_file = Path(__file__).parent / "testinput_bkgdswap.json"
    ...
    if choice == "1":
        ...
        # Save result
        output_file = output_dir / f"bkgd_sync_result_{timestamp}.json"
        ...
        if "error" not in result:
            ...
            # Process background swap specific results
            print("\n🌄 Processing background swap results...")
            tester.process_background_swap_results(result, output_dir, timestamp)
        else:
        ...
        if job_id:
            ...
            # Save result
            output_file = output_dir / f"bkgd_async_result_{timestamp}.json"
            tester.save_result(result, str(output_file))
