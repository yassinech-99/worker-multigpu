#!/usr/bin/env python3
"""
Runpod Serverless Endpoint Test Script for Character Detection (Whole Body)
This script sends a request to your deployed Runpod serverless endpoint using the testinput_chardetect.json file.
"""
...
class RunpodTester:
    ...
    def process_detection_results(self, result: Dict[str, Any], output_dir: Path, timestamp: int) -> None:
        """
        Process and save detection results (bounding boxes, masks, etc.).
        ...
        try:
            ...
            # Look for detection data in various possible formats
            detection_data = None
            if 'detection' in output:
                detection_data = output['detection']
            elif 'segs' in output:
                detection_data = output['segs']
            elif 'bbox' in output:
                detection_data = output['bbox']
            ...
            if detection_data:
                ...
                # Print summary if it's a list of detections
                if isinstance(detection_data, list):
                    print(f"🔍 Found {len(detection_data)} detection(s)")
...
def main():
    ...
    print("🧪 Runpod Character Detection (Whole Body) Endpoint Tester")
    print("=" * 60)
    ...
    # Configuration
    test_input_file = Path(__file__).parent / "testinput_chardetect.json"
    ...
    if choice == "1":
        ...
        if "error" not in result:
            ...
            tester.process_detection_results(result, output_dir, timestamp)
        else:
